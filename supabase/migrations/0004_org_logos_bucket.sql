-- Apply DROP + recreation if bucket already created.
...
create policy "org_logos_write" on storage.objects for insert with check (
  bucket_id = 'org-logos' and auth.role() = 'authenticated'
);
