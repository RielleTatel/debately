...
import { streamTeamsCsv } from '@/services/csv/reports/teams'
...
export async function GET(_req: NextRequest, ctx: { params: Promise<{ tournamentId: string }> }) {
  try {
    ...
    const stream = streamTeamsCsv(tournamentId)
    return new NextResponse(stream, {
      headers: {
        'content-type': 'text/csv; charset=utf-8',
        'content-disposition': `attachment; filename="teams-${tournamentId}.csv"`,
      },
    })
  } catch (e) {
